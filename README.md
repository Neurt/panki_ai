<img width="400" height="800" alt="image" src="https://github.com/user-attachments/assets/d8a9de12-835d-4111-b675-c33915f4fb3f" />
<img width="400" height="800" alt="image" src="https://github.com/user-attachments/assets/a3373856-0b9c-457f-98e0-b154f84813cd" />
...
 CPU  Cortex-A72 (CPU part `0xd08`), ARMv8.0-A. Features: `fp asimd evtstrm crc32 cpuid`. No dotprod or FP16. 
...
 QVAC SDK (npm)  `@qvac/sdk` 0.12.2. The prebuilt LLM addon is ARMv8.2 and will not run on the A72, so we use the source-built Fabric engine instead. 
 Node.js  v22.22.3 (QVAC needs 22.17 or newer) 
 Vulkan  V3D 4.2.14 (Mesa `v3dv`) 
 LLM (fast)  Qwen3-0.6B-Q4_0 (unsloth GGUF), about 7.5 tok/s, terse 
 LLM (quality)  Qwen2.5-1.5B-Instruct-Q4_K_M (Qwen GGUF), about 4 tok/s, roughly 2 min per query, full professional explanation 
 Server  `llama-server`, OpenAI-compatible API on port 11434 
 Messaging  MQTT. Either Mosquitto on the Pi (port 1883, LAN, fully offline) or HiveMQ Cloud (port 8883, TLS and auth) for cross-network access. Client is `python3-paho-mqtt`. The bridge takes `--broker`, `--broker-port`, `--username`, `--password`, `--tls`. 
 App  Flutter (Android) using `mqtt_client` with TLS and credentials. Connect tab (host, port, user, password, TLS toggle, all persisted) and a chatbot with saved history. 


Reference, taken from the QVAC docs: Linux support is Ubuntu 22 or newer, arm64 or x64, with a Vulkan runtime. The OpenAI-compatible server runs on `localhost:11434/v1/`. Delegated inference uses `providerPublicKey` and `fallbackToLocal`, with no topic or discovery phase. See https://docs.qvac.tether.io/
...
5. How a query flows
Phone publishes {"id": <uuid>, "message": "<product>"} to panki/ask.
Bridge retrieves the matching BPOM regulation chunks (by category/keywords).
Bridge builds the authoritative parameter list straight from the chunks' data (e.g. biscuits → ALT, kapang dan khamir, Salmonella, Pb, Cd, As, Hg — each cited).
Bridge asks llama-server (QVAC Fabric) to write a grounded explanation constrained to those chunks.
Bridge publishes the structured answer to panki/answer with the same id; the app matches it to the question and renders it in the chat.
...
git clone --depth 1 [https://github.com/tetherto/qvac-fabric-llm.cpp.git](https://github.com/tetherto/qvac-fabric-llm.cpp.git)
...
wget -O qwen2.5-1.5b-instruct-q4_k_m.gguf \
  ...
  [https://huggingface.co/Qwen/Qwen2.5-1.5B-Instruct-GGUF/resolve/main/qwen2.5-1.5b-instruct-q4_k_m.gguf](https://huggingface.co/Qwen/Qwen2.5-1.5B-Instruct-GGUF/resolve/main/qwen2.5-1.5b-instruct-q4_k_m.gguf)
